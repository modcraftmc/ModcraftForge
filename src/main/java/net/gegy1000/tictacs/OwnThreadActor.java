...
public final class OwnThreadActor<T> extends DelegatedTaskExecutor<T> {
    ...
    private OwnThreadActor(ITaskQueue<? super T, ? extends Runnable> queue, String name) {
    ...
    private static OwnThreadActor<Runnable> startActor(String name) {
        ITaskQueue.Single<Runnable> queue = new ITaskQueue.Single<>(new ArrayDeque<>());
