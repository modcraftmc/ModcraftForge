...
public final class ChunkUpgradeFuture implements Future<Unit> {
        ...
        while (true) {
            ...
            try {
                if (result == AcquireChunks.Result.OK) {
                    IChunk[] pollChunks = this.stepper.pollStep(waker, this.upgrade.entries, this.acquireChunks, currentStep);
                    if (pollChunks == null) {
                        return null;
                    }
                ...
                TicTacs.LOGGER.error("Failed to generate chunk at {}", this.entry.getPosition(), e);
                return this.returnUnloaded(currentStep);
            }
    ...
    private void notifyUpgrades(IChunk[] chunks, ChunkStep step) {
    ...
    public String toString() {
        ...
        display.append("upgrading ").append(this.entry.getPosition()).append(" to ").append(this.targetStep).append(": ");
