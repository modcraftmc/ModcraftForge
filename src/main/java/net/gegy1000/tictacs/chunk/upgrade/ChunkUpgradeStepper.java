...
final class ChunkUpgradeStepper {
    ...
    private final Future<IChunk>[] tasks;
    ...
    ChunkUpgradeStepper(ChunkUpgradeFuture parent) {
        ...
        this.chunks = kernel.create(IChunk[]::new);
    }
    ...
    IChunk[] pollStep(Waker waker, ChunkUpgradeEntries entries, AcquireChunks chunks, ChunkStep step) {
        Future<IChunk>[] tasks = this.tasks;
    ...
    private void openUpgradeTasks(ChunkUpgradeEntries entries, AcquireChunks chunks, ChunkStep step, Future<IChunk>[] tasks) {
        chunks.openUpgradeTasks(tasks, entry -> this.upgradeChunk(entry, entries, step));
    }

    private Future<IChunk> upgradeChunk(ChunkEntry entry, ChunkUpgradeEntries entries, ChunkStep step) {
        ContextView context = this.openContext(entry, entries, step);

        Future<IChunk> future = this.parent.controller.getUpgrader().runStepTask(entry, step, context);
    ...
    private ContextView openContext(ChunkEntry entry, ChunkUpgradeEntries entries, ChunkStep step) {
        ...
        ChunkPos targetPos = entry.getPosition();
    ...
    private TaskWithContext createTaskWithContext(Future<IChunk> future, ContextView context) {
    ...
    static class TaskWithContext implements Future<IChunk> {
        Future<IChunk> future;
        ContextView context;

        @Override
        public IChunk poll(Waker waker) {
            IChunk poll = this.future.poll(waker);
            if (poll != null) {
                this.release();
                return poll;
            }
    ...
    static class ContextView extends AbstractList<IChunk> {
        ...
        void open(
            ...
            if (TicTacs.DEBUG) {
                ...
                IChunk chunk = this.get(this.size() / 2);
                if (chunk == null) {
        ...
        @Override
        public IChunk get(int targetIndex) {
