...
class PrepareUpgradeFuture implements Future<Result<ChunkUpgrade>> {
    ...
    private volatile Future<IChunk>[] loadFutures;
    private volatile IChunk[] loadedChunks;
    ...
    public Result<ChunkUpgrade> poll(Waker waker) {
        ...
        while (true) {
            ...
            IChunk[] chunks = this.pollLoadChunks(waker, this.entries);
            if (chunks == null) {
                return null;
            }
    ...
    private void notifyChunkLoads(IChunk[] chunks) {
        ...
        for (int z = -radius; z <= radius; z++) {
            for (int x = -radius; x <= radius; x++) {
                ...
                if (entry.canUpgradeTo(this.fromStep)) {
                    IChunk chunk = chunks[kernel.index(x, z)];
    ...
    private ChunkStep tryStep(IChunk[] chunks) {
    ...
    private ChunkStep getMinimumStepFor(IChunk[] chunks) {
    ...
    private IChunk[] pollLoadChunks(Waker waker, ChunkUpgradeEntries entries) {
        ChunkUpgradeKernel kernel = entries.kernel;
        int radius = kernel.getRadius();

        if (this.loadFutures == null) {
            this.loadFutures = kernel.create(Future[]::new);
            this.loadedChunks = kernel.create(IChunk[]::new);
            ChunkUpgrader upgrader = this.controller.getUpgrader();
            ...
            for (int z = -radius; z <= radius; z++) {
                for (int x = -radius; x <= radius; x++) {
                    ...
                    ChunkPrimer chunk = entry.getProtoChunk();
                    if (chunk != null) {
