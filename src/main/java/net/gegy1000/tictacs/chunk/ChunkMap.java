...
public final class ChunkMap {
    ...
    private ChunkEntry createEntry(long pos, int level) {
        ...
        ChunkEntry unloadingEntry = (ChunkEntry) tacs.chunksToUnload.remove(pos);
        if (unloadingEntry != null) {
            unloadingEntry.setChunkLevel(level);
            return unloadingEntry;
        }

        return new ChunkEntry(new ChunkPos(pos), level, this.world.getLightManager(), tacs.field_219263_q, tacs);
    }
    ...
    final class Primary implements ChunkAccess {
        ...
        public void putEntry(ChunkEntry entry) {
            long pos = entry.getPosition().asLong();
